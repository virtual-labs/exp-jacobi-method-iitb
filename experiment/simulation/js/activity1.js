...
function activity1() {
    ...
    <div style='margin-top: 2vw;'>
        <br>
        <h4 class="center-text fs-20px fw-600">System of Linear Equations: Jacobi Method</h4>
...
let matrix_a_inputs = get_matrix_html_with_title(3, 3, [
    [
...
let matrix_c_inputs = get_matrix_html_with_title(3, 1, [
    [
...
function start_act1() {
    ...
    let btn_text = get_collapse_btn_text("Pivoting", "tb1-box");
    let text = `
    ${btn_text}
...
function verify_matrix_a_c() {
    ...
    for (let i = 0; i < 3; i++) {
        for (let j = 0; j < 3; j++) {
            ...
            if (!verify_values(parseFloat(inp_a.value), mat_a[i][j])) {
                alert(`incorrect value in 3 x 3 matrix for a[${i}][${j}]`);
                return;
            }
        }
    }
    ...
    for (let i = 0; i < 3; i++) {
        ...
        if (!verify_values(parseFloat(inp_c.value), mat_c[i][0])) {
            alert(`incorrect value in 3 x 1 matrix for b[${i}]`);
            return;
        }
    }
    btn.remove();
...
function render_pivoted_a_c() {
    ...
    div.innerHTML = `${get_matrix_html_with_title(3, 3, mat_a, "A", 'inline-block', "40%")} &nbsp; <span style='font-size: 30px;'>. X</span>  &nbsp; =  &nbsp; ${get_matrix_html_with_title(3, 1, mat_c, "C", 'inline-block', "15%")}`;
}
activity1();
